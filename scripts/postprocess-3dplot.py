...
if __name__ == "__main__":
    ...
    # best CI
    best_ci = np.zeros(qs_ref.shape[:-1], dtype=FLOAT_TYPE)
    best_ci.fill(-10**9)
    ...
    with tarfile.open(INFILE, mode="r") as archive:
        for file_meta in archive:
            ...
            S, mult = shannon_entropy_from_lambda(
                content["lambda"], KSYS, 1.0, qs_ref
            )
            S_a, mult_a = shannon_entropy_from_lambda(
                content["lambda_a"], KSYS, 2.0 ** -KENV, qs_ref
            )
    ...
    np.savez_compressed(OUTFILE, ci=best_ci, graph=best_graph)
